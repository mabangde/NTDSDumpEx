...
void usage() {
	printf(
		"usage: ntdsdumpex.exe <-d ntds.dit> <-k HEX-SYS-KEY | -s system.hiv |-r> [-o out.txt] [-h] [-m] [-p] [-u] [-c]\n"
...
int main(int argc, char* argv[]) {
	...
	for (int i = 1; i < argc; i++) {
		if ((stricmp(argv[i], "-d") == 0) && (argc > i + 1)) {
			...
			hasClearText = TRUE;
			printf("[+]ClearText option enabled\n");
		}
		else {
			usage();
		}
	...
	if (ntds->Load(ditfile)) {
		if (ntds->GetPEKey(systemkey, passwordkey)) {
			...
			ntds->SetDumpOptions(hasHistory, hasMachines, hasProfile, hasClearText);
			ntds->SetOutputFile(out);
			...
			if (ntds->GetHashes(chFmt, hasHistory, hasInactive, hasMachines, hasProfile,
				out, &dwAccounts, &dwMachines, &dwEntries, &dwHistory)) {
				clock_t c2 = clock();
